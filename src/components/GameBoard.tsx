...
const GameBoard: React.FC<GameBoardProps> = React.memo(
  ({
    ...
    const renderTetromino = (): void => {
      ...
      const newGrid: Grid = gridRef.current.map((row) =>
        row.map((cell) => ({ ...cell }))
      );
      ...
      for (let i = 0; i < height; i++) {
        for (let j = 0; j < width; j++) {
          if (tetromino.shape[i][j]) {
            // 直下のテトロミノを設定する。
            newGrid[dropPosition.y + i][dropPosition.x + j] = {
              filled: 1,
              type: "dummy",
            };
            // 現在のテトロミノを設定する。
            // 直下の後に設定して確実に色を反映させる。
            newGrid[positionRef.current.y + i][positionRef.current.x + j] = {
              filled: 1,
              type: tetromino.type,
            };
          }
    ...
    const checkPerfect = (): boolean => {
      ...
      gridRef.current.map((rows) =>
        rows.map((cell) => (countCells += cell.filled))
      );
      if (countCells !== 0) {
        clearPerfect = false;
      }
    ...
    const rotateTetromino = (direction: string): void => {
      ...
      const newTetromino: Tetromino = {
        type: activeTetromino.current.type,
        shape: rotated,
      };
      const newPosition: Position | undefined = calculateSRSPosition(
        newTetromino,
        positionRef.current,
        direction
      );
    ...
    const holdTetromino = (): void => {
      ...
      activeTetromino.current = {
        ...
        shape: heldTetromino.shape.map((row) => [...row]),
      };
      positionRef.current = initializePosition();
      rotationAngleRef.current = 0;
      hasHeldRef.current = true;
    ...
    const checkGameOver = (): boolean => {
      ...
      for (let i = 0; i < height; i++) {
        for (let j = 0; j < width; j++) {
          ...
          if (
            tetromino.shape[i][j] &&
            gridRef.current[positionRef.current.y + i][
              positionRef.current.x + j
            ].filled
          ) {
            return true;
          }
    ...
    const handleEndOfTurn = (): void => {
      if (isGameOver) return; // ゲームオーバー時は処理を停止
      ...
      initializeNewTetromino();
      if (checkGameOver()) {
        setIsGameOver(true); // ゲームオーバー状態をセット
        onGameOver();
        return;
      }
    ...
    // 基本的なゲームの流れ。
    useEffect(() => {
      if (isGameOver) return; // ゲームオーバー時はインターバルを作成しない
      ...
      const intervalDuration = hasLanded
        ? DEFAULT_INTERVAL
        : Math.max(50, DEFAULT_INTERVAL - (level - 1) * 100);
    ...
    }, [level, hasLanded, isGameOver]); // isGameOverを依存配列に追加
    ...
    useEffect(() => {
      const handleKeyPress = (e: KeyboardEventInit) => {
        if (isGameOver || isLockedRef.current) return; // ゲームオーバー時は操作を無効化
    ...
    return (
      <div className="relative bg-gray-800 p-4 rounded-lg border-2 border-purple-500 shadow-lg mb-4">
        <div className="bg-gray-900 p-1 rounded border border-gray-700">
